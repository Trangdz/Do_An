...
export default function RepayModalWithDebug({
  open,
  onClose,
  token,
  poolAddress,
  ...
  const handleRepay = async () => {
    ...
    try {
      ...
      if (amount === "MAX") {
        ...
        // Execute repay with debug helper
        const result = await executeRepayWithDebug(signer, token.address, amountBN, token.decimals);
            ...
            <Collapsible open={showDebug} onOpenChange={setShowDebug}>
              <CollapsibleTrigger asChild>
                <Button variant="outline" className="w-full">
                  {showDebug ? 'Hide' : 'Show'} Debug Info ({debugSteps.length} steps)
                </Button>
              </CollapsibleTrigger>
                ...
                <div className="bg-gray-100 p-3 rounded-lg space-y-2">
                  ...
                  {debugSteps.map((step, index) => (
                    <div key={index} className="text-xs">
                      ...
                      <div className="ml-8 text-gray-600">{step.details}</div>
                      {step.data && (
                        <div className="ml-8 text-xs text-gray-500 bg-gray-50 p-2 rounded mt-1">
