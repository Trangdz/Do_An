...
`;
  ...
  // Auto-update frontend contracts.ts with aggregator addresses
  const contractsTsPath = path.join(__dirname, "../lendhub-frontend-nextjs/src/config/contracts.ts");
  if (fs.existsSync(contractsTsPath)) {
    let contractsContent = fs.readFileSync(contractsTsPath, "utf8");
    
    // Update each aggregator address
    contractsContent = contractsContent.replace(
      /aggregatorAddress:\s*['"]0x[a-fA-F0-9]+['"],\s*\/\/\s*ETH aggregator/,
      `aggregatorAddress: '${aggregators.ETH}', // ETH aggregator`
    );
    contractsContent = contractsContent.replace(
      /aggregatorAddress:\s*['"]0x[a-fA-F0-9]+['"],\s*\/\/\s*WETH aggregator/,
      `aggregatorAddress: '${aggregators.WETH}', // WETH aggregator`
    );
    contractsContent = contractsContent.replace(
      /aggregatorAddress:\s*['"]0x[a-fA-F0-9]+['"],\s*\/\/\s*USDC aggregator/,
      `aggregatorAddress: '${aggregators.USDC}', // USDC aggregator`
    );
    contractsContent = contractsContent.replace(
      /aggregatorAddress:\s*['"]0x[a-fA-F0-9]+['"],\s*\/\/\s*DAI aggregator/,
      `aggregatorAddress: '${aggregators.DAI}', // DAI aggregator`
    );
    contractsContent = contractsContent.replace(
      /aggregatorAddress:\s*['"]0x[a-fA-F0-9]+['"],\s*\/\/\s*LINK aggregator/,
      `aggregatorAddress: '${aggregators.LINK}', // LINK aggregator`
    );
    
    fs.writeFileSync(contractsTsPath, contractsContent);
    console.log("✅ Updated frontend contracts.ts with aggregator addresses");
  } else {
    console.log("⚠️  contracts.ts not found, skipping update");
  }
