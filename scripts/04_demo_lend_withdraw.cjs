...
async function main() {
  ...
  tx = await pool.accruePublic(DAI, { gasLimit: 1000000 }); await tx.wait();
  ...
  tx = await pool.withdraw(DAI, withdrawReq, { gasLimit: 1000000 });
  ...
  tx = await pool.lend(USDC, lendAmountUSDC, { gasLimit: 1000000 }); await tx.wait();
  console.log("Lended 6,000 USDC (6 decimals).");

  // Accrue & Withdraw USDC 7,000 để thấy clamp (vì chỉ có 6,000 + lãi)
  await (await pool.accruePublic(USDC, { gasLimit: 1000000 })).wait();
  const withdrawUSDCReq = ethers.parseUnits("7000", usdcDec);
  await (await pool.withdraw(USDC, withdrawUSDCReq, { gasLimit: 1000000 })).wait();
