...
async function main() {
  ...
  // Try very small withdraw
  const tinyWithdraw = ethers.parseUnits("1", daiDec);
  console.log("Trying to withdraw 1 DAI...");
  
  try {
    const tx = await pool.withdraw(DAI, tinyWithdraw, { gasLimit: 1000000 });
    await tx.wait();
    console.log("✅ Tiny withdraw successful!");
  } catch (error) {
    console.log("❌ Tiny withdraw failed:", error.message);
  }
}
