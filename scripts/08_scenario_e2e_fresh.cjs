...
async function main() {
  console.log("🎬 Starting Fresh E2E Scenario...");
  console.log("============================================================");
  ...
  const feed = new ethers.Contract(ETH_USD_FEED, feedAbi, deployer);
  ...
  pool.on("ReserveDataUpdated", (asset, Uwad, liqRate, borRate) => {
    const U = Number(Uwad) / 1e16;
    const borrowAPR = (Number(borRate) * 365 * 24 * 3600) / 1e27 * 100;
    const supplyAPR = (Number(liqRate) * 365 * 24 * 3600) / 1e27 * 100;
  ...
  try {
    ...
    if (userDaiBal < ethers.parseUnits("20000", daiDec)) {
