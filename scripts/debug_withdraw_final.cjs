...
async function main() {
  ...
  // Try withdraw with different amounts
  const amounts = [1, 100, 1000, 10000];
  
  for (const amount of amounts) {
    const withdrawAmount = ethers.parseUnits(amount.toString(), daiDec);
    console.log(`\nTrying to withdraw ${amount} DAI...`);
    ...
    try {
      ...
      await tx.wait();
      console.log(`✅ Withdraw ${amount} DAI successful!`);
      break; // Stop on first success
    } catch (error) {
      console.log(`❌ Withdraw ${amount} DAI failed:`, error.message);
    }
  }
}
