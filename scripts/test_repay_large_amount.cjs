...
async function main() {
    console.log("🧪 Testing Repay with Large Amount (1M tokens)");
    ...
    try {
        ...
        if (userReserveBefore.borrow.principal > 0n) {
            ...
            if (balance >= userReserveBefore.borrow.principal) {
                ...
                // Use large amount (1M USDC) instead of MaxUint256
                const largeAmount = ethers.parseUnits("1000000", 6); // 1M USDC
                console.log("\nUsing large amount (1M USDC) for repay all...");
                console.log("Large amount:", largeAmount.toString());
                console.log("Formatted:", ethers.formatUnits(largeAmount, 6), "USDC");
                
                // Approve large amount
                console.log("\nApproving large amount...");
                const approveTx = await usdc.approve(LendingPoolAddress, largeAmount);
                await approveTx.wait();
                console.log("✅ Approval successful");
                
                // Try to repay with large amount
                console.log("\nAttempting repay with large amount...");
    ...
    console.log("\n💡 Key Points:");
    console.log("1. Large amount (1M tokens) is safer than MaxUint256");
    console.log("2. Contract should cap to actual debt amount");
